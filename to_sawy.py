...
def final(funs,xa,ya,za,itr):
    ...
    if function.find("r")!=-1:
        tv=3
        print(tv)
    elif function.find("z")!=-1:
        tv=2
        print(tv)
    else:
        tv=1
    ...
    if tv==1:
        ...
        func =convert_to_function(function)
        x_values=x_array
        y_values=y_array
        ...
        predictor(func,x_values,y_values,t,n)    
        ...
        def predictor(F,R,Q,Z,n):
            # Define the ODE function
            if len(R)>3 and len(Q)>3 and len(Z)>3:
                if len(R) == len(Q) ==len(Z):
                    z= x_values[-1]-x_values[-2]
                    x_end=x_values[-1]+z
                    ...
                    # we interpoleted to check things in between  the values because we found out at first we substuited wrong 
                    #so we used inter polition and just didnt want to change it
                    #geting the appproximnate of the next point for refrence
                    y_interpolated = interp1d(t_initial, y_initial, kind='linear', fill_value="extrapolate")
                    ...
                    #W is the the value of the y  with the y  and x that you want to calculate value of the function 
                    W = np.zeros(N + 1)
                    #v is the the value of the  with the z  and x that you want to calculate value of the function 
                    V=np.zeros(N+1)
                    # Output file setup
                    ...
                    #corrector step
                    for i in range(n):
                        Part1 = 9.0 * F(T[-1], Wn,Vn) + 19.0 * F(T[-2], W[-2],V[-2]) - 5.0 * F(T[-3], W[-3],V[-3]) + F(T[-4], W[-4],V[-4])
                        #print(F(T[-1], Wn,Vn))
                        W[-1] = W[-2] + H * (Part1) / 24.0
                        Wn=W[-1]
...
input_list = input_string.split()
    ...
    # Convert the substrings to integers
xa = [float(element) for element in input_list]
...
input_list = input_string.split()
    ...
    # Convert the substrings to integers
ya = [float(element) for element in input_list]
input_string = input()
...
input_list = input_string.split()
    ...
    # Convert the substrings to integers
za = [float(element) for element in input_list]

ne=input()
itr=int(ne)

#funs is our function  as string
#xa is the x array
#ya is the y array
#za is the z array
#itr is the number of itrations


final(funs,xa,ya,za,itr)
